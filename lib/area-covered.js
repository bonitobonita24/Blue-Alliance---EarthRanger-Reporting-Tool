...
function boundaryId(b) {
  return b?.id ?? b?.properties?.id;
}

function boundaryName(b) {
  return b?.name ?? b?.properties?.name;
}
...
function accumulatePatrol({ track, boundaries, patrolTotalHrs, aggregates }) {
  ...
  for (let i = 1; i < coordinates.length; i++) {
    ...
    const hitId = boundaryId(hit);
    if (hitId == null) continue;

    perBoundaryKm[hitId] = (perBoundaryKm[hitId] || 0) + segKm;
    ...
    if (hasTimestamps) {
      ...
      perBoundaryHrs[hitId] = (perBoundaryHrs[hitId] || 0) + hrs;
    }
  }
  ...
  for (const [bid, km] of Object.entries(perBoundaryKm)) {
    const boundary = boundaries.find((b) => String(boundaryId(b)) === String(bid));
    if (!aggregates[bid]) {
      aggregates[bid] = {
        boundary_name: boundaryName(boundary) || bid,
        coverage_patrols: 0,
        coverage_km: 0,
        coverage_hrs: 0,
